...
export const tooltipStyles = tv({
  slots: {
    base: "flex aspect-video flex-col justify-center rounded-md border bg-background text-xs shadow-lg",
  ...
  variants: {
    ...
    indicator: {
      ring: {
        entryIndicator: "size-1.5 rounded-full ring ring-[var(--entry-indicator-color)]",
      },
      circle: {
        entryIndicator: "size-2.5 rounded-full bg-[var(--entry-indicator-color)]",
      },
      square: {
...
type TooltipProps<TValue extends ValueType, TName extends NameType> = TooltipContentProps<TValue, TName> &
      ...
      <div className={content()}>
        {payload.map((entry, index) =>
          formatter && entry?.value !== undefined && entry.name ? (
            ...
            <div key={`${entry.name || entry.dataKey || "value"}`} className={entryClasses()}>
              ...
              {entry.value && <span className={entryValue()}>{entry.value}</span>}
            </div>
          ),
        )}
      </div>
...
const ChartLegend = ({ value, className }: React.ComponentPropsWithRef<"span"> & { value: string }) => {
  return <span className={cn("text-sm font-medium text-muted-foreground", className)}>{value}</span>
}
